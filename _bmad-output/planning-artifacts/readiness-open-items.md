...
| **O-3.1-1** | 2026-05-31 | **SHAPE CONHECIDO (3.3 spot-check):** o `template[]` real é Meta-component (`{type, sub_type, index, parameters:[{type, text\|payload}]}`, `name`=template, `openTicket`/`queueId` numéricos) — documentado em `whatsapp-templates-utility.md`. O `payload-schema.ts`/`buildBody` da 3.1 estão errados (dry-run → sem dano live). | **reconciliar o adapter da 3.1** (consumir `template-catalog.ts`) — follow-up dedicado (Q-3.3-1=b) |
| **O-3.3-1** | 2026-05-31 | buttons do doc/catálogo (`p1_mudar_rumo`, `fin_rever`, `fin_bloquear`, s1/s2/hb/rel) divergem do `PAYLOAD_MAP` (1.a.4: `p1_pausar_agora`, `fin_pedir_mudancas`, `fin_rejeitar`; sem s1/s2/hb/rel). | **Story 3.4** (parser inbound) — reconciliar `PAYLOAD_MAP` ↔ catálogo |
...
| **3.4** | arquitectura: "Inbound webhook (clihelper → HDD)" | realidade (memória n8n-topology + ao_subset): **n8n → HDD** (n8n=trust boundary; trata X-Hub-Signature; HDD confia via Bearer) | diverg. **resolvida p/ memória** (Q-3.4-1=a); texto da arquitectura desactualizado |
